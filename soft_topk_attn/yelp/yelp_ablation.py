...
# --- FAISS candidate retriever (optional, same as PEMS) ---
try:
...
@torch.no_grad()
def select_topk(
    Q_batch: torch.Tensor,     # [A, d_out]  (already W_q applied)
    topm: int,
    emb_nodes: torch.Tensor,   # [N, d_emb]
    attention,                 # needed for W_k + normalize_qk
) -> torch.Tensor:
    """
    Exact top-k candidate selection by full sort
    Returns:
      cand_batch: LongTensor [A, topm]
    """
    N = int(emb_nodes.size(0))
    topm = int(min(int(topm), N))

    # Keys for all nodes
    K_all = attention.W_k(emb_nodes).detach()  # [N, d_out]
    ...
    # Scores [A, N]
    d_out = int(Q_batch.size(-1))
    scores = (Q_batch @ K_all.t()) / math.sqrt(d_out)

    # Exact topm by sorting all N scores per anchor (descending)
    # argsort is exact; ties are handled consistently by PyTorch's sort implementation.
    order = torch.argsort(scores, dim=1, descending=True)  # [A, N]
    cand_batch = order[:, :topm].contiguous()              # [A, topm]
...
def train_one_run(args, use_faiss: bool):
    ...
    for ep in range(args.epochs_warmup):
        ...
        # FAISS warmup policy:
        # - disable FAISS for early warmup epochs (index vectors are still very unstable)
        # - enable FAISS ONLY in the last warmup epoch, using topm_init for initialization
    ...
    for idx in range(W, S - Hmax):
        ...
        label_cache_cpu: Dict[Tuple[int, int, int], Optional[int]] = {}
        # key = (t_now, horizon, anchor_b) -> 0/1 or None
        ...
        for h in Hs:
            ...
            loss_h, m_h, inf_time = eval_one_snapshot(
                ...
                retriever=retriever,
                label_cache_cpu=label_cache_cpu,
            )
            #t_eva_dist = _now() - t_eva_start
    ...
    t_eva_persnap = t_eva_total / t_eva_count
    ...
    return warmup_time, rolling_time, summary, t_eva_persnap


def _get_h_idx(h: int) -> int:
    return 0 if int(h) == 1 else 1

def _sample_anchors_business_only(Nb: int, k: int, device: torch.device, seed: int, salt: int) -> torch.Tensor:
    # sample on CPU with a local Generator for determinism across runs
    cand_cpu = torch.arange(Nb, device="cpu")
    if k is None or k < 0 or k >= cand_cpu.numel():
        return cand_cpu.to(device)

    g = torch.Generator(device="cpu")
    g.manual_seed(int(seed) + int(salt))

    perm = torch.randperm(cand_cpu.numel(), generator=g)  # CPU perm, deterministic
    return cand_cpu[perm[:k]].to(device)
...
def train_step_one_snapshot(
    *,
    ...
    beta_mixup: float,
    use_faiss: bool,
    retriever: Optional[object],  # FAISS index over K=W_k(emb_nodes)
    faiss_use_topm_init: bool = False,      # True only in last warmup epoch
    ...
    anchor_indices = _sample_anchors_business_only(
        Nb=Nb,
        k=args.anchors_train,
        device=device,
        seed=args.seed,
        salt=1000003 * int(snap.t) + 1,  # +1 = "train"
    )
    ...
    if use_faiss:
        ...
        # build/update index over K = W_k(emb_nodes)
...
def eval_one_snapshot(
    *,
    ...
    for _i, anchor_b in enumerate(anchor_indices.tolist()):
        ...
        key = (int(snap.t), int(horizon), int(anchor_b))
        y = label_cache_cpu.get(key, "MISS")
        if y == "MISS":
            y0 = label_peer_quantile_future_count(
                ds=ds,
                t_now=snap.t,
                horizon=horizon,
                anchor_b=anchor_b,
            ...
            y = None if y0 is None else int(y0)
            label_cache_cpu[key] = y
        if y is None:
            continue
    ...
    max_recall = min(1.0, args.k_eval / max(1.0, total_pos))
    #print("[eval] used=", used, "pos=", total_pos, "max_recall@K=", max_recall)
...
def parse_args():
    ...
    p.add_argument("--anneal", type=float, default=0.95)
    p.add_argument("--print_every", type=int, default=1)

    # attention params
    p.add_argument("--tau", type=float, default=0.8)
    p.add_argument("--init_k_frac", type=float, default=0.001)
    p.add_argument("--k_min", type=float, default=0.0001)
    p.add_argument("--k_max", type=float, default=0.1)
    p.add_argument("--k_abs_min", type=int, default=5)
    p.add_argument("--k_abs_max", type=int, default=20)

    # loss weights
    p.add_argument("--beta_div", type=float, default=0.15)
    p.add_argument("--beta_metric", type=float, default=0.35)
    ...
    p.add_argument('--faiss_c', type=float, default=20.0, help='Candidate multiplier: topm ~= learned_k * c (after warmup).')
    ...
    p.add_argument('--faiss_topm_min', type=int, default=32, help='Minimum topm to request from FAISS.')
    p.add_argument('--faiss_topm_max', type=int, default=256, help='Maximum topm to request from FAISS.')
...
def main():
    ...
    warm0, roll0, summ0, eval_time0 = train_one_run(args, use_faiss=False)

    print("\n===== Summary =====")
    print(f"No-FAISS: warmup_time={warm0:.2f}s rolling_time={roll0:.2f}s inference_time_per_snapshot={eval_time0}s")
    ...
    if bool(getattr(args, 'use_faiss', False)):
        warm1, roll1, summ1, eval_time1 = train_one_run(args, use_faiss=True)
        print(f"FAISS: warmup_time={warm1:.2f}s rolling_time={roll1:.2f}s inference_time_per_snapshot={eval_time1}s")
        for h, m in summ1.items():
            print(
                f"  horizon={h} avg_loss={m['avg_loss']:.6f} "
                f"AUC={m['auc']:.3f} ROC-AUC={m['roc_auc']:.3f} Average Precision={m['ap']:.3f} F1={m['F1']:.3f} "
                f"Precision@{args.k_eval}={m['precision@k']:.3f} Recall@{args.k_eval}={m['recall@k']:.3f} "
