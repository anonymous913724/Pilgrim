...
def visualize(
    ...
    t_vis: int,
    device: torch.device,
    output_path: str = "pems_baseline_tsne.png",
):
    model.eval()
    ...
    plt.title("PEMS Baseline tSNE embeddings (PCA->tSNE: 32->16->2)")
    plt.legend()
    plt.tight_layout()
...
def train_and_visualize_baseline(
    ...
    # After training, visualize node embeddings with tSNE on the last training snapshot.
