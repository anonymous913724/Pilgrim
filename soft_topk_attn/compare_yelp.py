...
WARMUP_SNAPSHOTS = 1          # must be >= 1; typical: max(1, LAGS)
EPOCHS_WARMUP = 1             # number of passes over warmup snapshots
...
# Task choice
# =========================
# TASK1: "region rating disparity" in the future month:
#   disparity = max(mean_rating_per_business) - min(mean_rating_per_business) within k-hop business region
# TASK2: "elite user vs non-elite user disagreement" in the future month:
#   diff = |mean_rating_elite - mean_rating_nonelite| within (biz subsset + users who reviewed them)
TASK = "TASK2"  # "TASK1" or "TASK2"

THRESH_TASK1 = 3.0 #3.5
THRESH_TASK2 = 0.3 #1.0
#THRESH_TASK2 = 0.05
# ---------------- Task2A evidence (elite-share surge) ----------------
TASK2A_MIN_REVIEWS_NOW = 10
TASK2A_MIN_REVIEWS_FUT = 10
# =========================
# Evidence filter switch
# =========================
EVIDENCE_FILTER_ON = True   # turn on/off evidence filtering
...
# ============================================================
# Yelp labels: computed from future month
# ============================================================
@torch.no_grad()
def label_task1_region_rating_disparity(
    ...
    """
    Task1 label = 1 if disparity > threshold, else 0.
    disparity computed in FUTURE month within biz_subset.
    """
    biz_subset = biz_subset.to(device=device)
    store = data_future["user", "rev", "business"]
    eidx = store.edge_index.to(device=device)

    if eidx.numel() == 0 or (not hasattr(store, "edge_rating")) or store.edge_rating is None or store.edge_rating.numel() == 0:
        #vals = data_future["business"].x[biz_subset, 1].to(device=device)
        x_biz = data_future["business"].x  # CPU tensor
        idx = biz_subset.to("cpu") if biz_subset.is_cuda else biz_subset
        vals = x_biz[idx, 1].to(device=device)  # move only the result to GPU

        return ((vals.max() - vals.min()) > threshold).long()

    er = store.edge_rating.to(device=device).float()
    hit = torch.isin(eidx[1], biz_subset)
    if hit.sum().item() == 0:
        #vals = data_future["business"].x[biz_subset, 1].to(device=device)
        x_biz = data_future["business"].x  # CPU tensor
        idx = biz_subset.to("cpu") if biz_subset.is_cuda else biz_subset
    ...
    Nb = int(data_future["business"].x.size(0))
    sum_r = torch.zeros((Nb,), device=device, dtype=torch.float)
    cnt_r = torch.zeros((Nb,), device=device, dtype=torch.float)
    sum_r.scatter_add_(0, b, r)
    cnt_r.scatter_add_(0, b, torch.ones_like(r))

    mean_r = torch.zeros_like(sum_r)
    nz = cnt_r > 0
    mean_r[nz] = sum_r[nz] / cnt_r[nz]

    vals = mean_r[biz_subset]
    nz_sub = nz[biz_subset]
    if nz_sub.any():
        vals = vals[nz_sub]
    if vals.numel() == 0:
...
@torch.no_grad()
def _task2A_has_enough_evidence(
    data_now: HeteroData,
    data_future: HeteroData,
    biz_subset: torch.Tensor,
    device: torch.device,
    ...
    """
    Require enough total reviews in the region at BOTH now and future,
    otherwise delta elite-share is too noisy.
    """
    biz_subset = biz_subset.to(device=device)

    def count_reviews(data: HeteroData) -> int:
        store = data["user", "rev", "business"]
        eidx = store.edge_index.to(device=device)
        if eidx.numel() == 0:
            return 0
        hit = torch.isin(eidx[1], biz_subset)
    ...
    n_now = count_reviews(data_now)
    if n_now < int(TASK2A_MIN_REVIEWS_NOW):
        return False

    n_fut = count_reviews(data_future)
    if n_fut < int(TASK2A_MIN_REVIEWS_FUT):
        return False
...
def label_task2_elite_surge(
    ...
    """
    Task2A label = 1 if elite share in the region increases by > threshold
    from now -> future.

    elite_share = (# elite reviews in subset) / (# total reviews in subset + eps)
    delta = elite_share_future - elite_share_now
    """
    biz_subset = biz_subset.to(device=device)

    def elite_share(data: HeteroData) -> torch.Tensor:
        store = data["user", "rev", "business"]
        eidx = store.edge_index.to(device=device)
        ...
        hit = torch.isin(eidx[1], biz_subset)
        n_total = int(hit.sum().item())
        if n_total == 0:
            return torch.tensor(0.0, device=device)

        u = eidx[0][hit]  # users who reviewed businesses in subset

        elite = data["user"].x[:, ELITE_CHANNEL].to(device=device).float()
        elite_u = elite[u] > 0.5
        n_elite = elite_u.sum().float()

        return n_elite / (float(n_total) + eps)

    s_now = elite_share(data_now)
    s_fut = elite_share(data_future)
    delta = s_fut - s_now

    return (delta > float(threshold)).to(dtype=torch.long)


@torch.no_grad()
def label_task2_elite_disagreement(
    data_future: HeteroData,
    biz_subset: torch.Tensor,
    threshold: float,
    device: torch.device,
    ...
    """
    Task2 label = 1 if elite vs non-elite mean-rating difference > threshold.
    """
    biz_subset = biz_subset.to(device=device)
    store = data_future["user", "rev", "business"]
    eidx = store.edge_index.to(device=device)
    ...
    u = eidx[0][hit] # users who reviewed these businesses
    r = er[hit] # ratings of those reviews

    elite = data_future["user"].x[:, ELITE_CHANNEL].to(device=device).float()
    elite_u = elite[u] > 0.5

    m_elite = r[elite_u].mean() if elite_u.any() else torch.tensor(0.0, device=device)
    m_non = r[~elite_u].mean() if (~elite_u).any() else torch.tensor(0.0, device=device)
    diff = torch.abs(m_elite - m_non)

    return (diff > threshold).long()
...
def eval_epoch(
    ...
    for s in range(t_start, t_end_eff):
        ...
        used = 0 # count for used anchor

        for _i, anchor_idx in enumerate(anchor_indices.tolist()):
            subset = khop_subsets[anchor_idx].to(device=device)
            # ---------------- Evidence filter (Task2A) ----------------
            if EVIDENCE_FILTER_ON and TASK.upper() != "TASK1":
                if not _task2A_has_enough_evidence(data_now, data_future, subset, device):
                    continue
            # ----------------------------------------------------------
            used += 1

            if TASK.upper() == "TASK1":
                y_class = label_task1_region_rating_disparity(data_future, subset, THRESH_TASK1, device)
            else:
                y_class = label_task2_elite_disagreement(data_future, subset, THRESH_TASK2, device)
                #y_class = label_task2_elite_surge(data_now, data_future, subset, THRESH_TASK2, device)
...
def eval_one_snapshot(
    *,
    ...
    for _i, anchor_idx in enumerate(anchor_indices.tolist()):
        ...
        # --------- Evidence filter (Task2A) ----------
        if EVIDENCE_FILTER_ON and TASK.upper() != "TASK1":
            data_now = data_list[month_idx]  # ADD THIS
            if not _task2A_has_enough_evidence(data_now, data_future, subset, device):
                continue
        # -------------------------------------------
        ...
        loss_sum += float(ce(logits2.view(1, 2), y_class.view(1)).item())
