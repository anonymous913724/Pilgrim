...
class QKOnlySoftTopKAttention(nn.Module):
    ...
    def __init__(
            ...
            eps: float = 1e-12,
            # =========================
            # CHANGED: optional absolute-k bounds (e.g., 5..20).
            # If provided, k is learned directly in [k_abs_min, k_abs_max] (independent of N_valid).
            # =========================
            k_abs_min: Optional[float] = None,
            k_abs_max: Optional[float] = None,
    ):
        super().__init__()
        self.d_out = d_out
        self.tau = float(tau)
        ...
        # =========================
        # CHANGED: store absolute-k bounds (optional)
        # =========================
        self.k_abs_min = None if k_abs_min is None else float(k_abs_min)
        self.k_abs_max = None if k_abs_max is None else float(k_abs_max)
        if (self.k_abs_min is None) != (self.k_abs_max is None):
            raise ValueError('k_abs_min and k_abs_max must be both set or both None')
        if self.k_abs_min is not None:
            assert self.k_abs_min > 0 and self.k_abs_max >= self.k_abs_min
    ...
    # =========================
    # CHANGED: helper for computing k (either absolute bounds or fraction-of-valid)
    # =========================
    def _compute_k(self, N_valid: int) -> torch.Tensor:
        u = torch.sigmoid(self.k_logit)  # in (0,1)
        if self.k_abs_min is not None:
            k = self.k_abs_min + (self.k_abs_max - self.k_abs_min) * u
            return torch.clamp(k, 0.0, float(N_valid))
        # original behavior: bounded fraction in [k_min, k_max] times N_valid
        k_frac = self.k_min + (self.k_max - self.k_min) * u  # in (k_min, k_max)
        return k_frac * float(N_valid)

    # =========================
    # CHANGED: candidate-only forward (runs attention + soft-topk on M candidates instead of all N)
    # This is used with FAISS candidates to avoid O(N) work per anchor.
    # =========================
    def forward_candidates(
            self,
            emb_q: torch.Tensor,
            emb_a: torch.Tensor,
            emb_nodes_cand: torch.Tensor,  # [M, d_in]
            node_mask: Optional[torch.Tensor] = None,  # [M] optional mask inside candidates
            tau: Optional[float] = None,
            return_intermediates: bool = False,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        if tau is None:
            tau = self.tau
        tau_t = torch.tensor(float(tau), device=emb_nodes_cand.device, dtype=emb_nodes_cand.dtype)

        qa = emb_q + emb_a
        Q = self.W_q(qa)  # [d_out]
        K = self.W_k(emb_nodes_cand)  # [M, d_out]

        if self.normalize_qk:
            Q = F.normalize(Q, dim=-1)
            K = F.normalize(K, dim=-1)

        scores = torch.matmul(K, Q) / math.sqrt(self.d_out)  # [M]
        if node_mask is not None:
            scores = scores.masked_fill(~node_mask.bool(), float('-inf'))
        attn = F.softmax(scores, dim=0)  # [M]
        ...
        k = self._compute_k(N_valid)  # CHANGED

        theta = self._solve_theta_newton(attn, k, tau_t, valid)
        soft_mask = torch.sigmoid((attn - theta) / tau_t)  # [M]
        if node_mask is not None:
            soft_mask = soft_mask * node_mask.to(dtype=soft_mask.dtype)

        w = (soft_mask * attn).unsqueeze(-1)  # [M,1]
        context = torch.sum(w * emb_nodes_cand, dim=0)  # [d_in]

        if return_intermediates:
            return context, attn, soft_mask, scores, Q, K, theta, k
        return context, attn, soft_mask
