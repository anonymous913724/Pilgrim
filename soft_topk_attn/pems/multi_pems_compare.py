...
from soft_topk_attn.models.attention_layer_alt import QKOnlySoftTopKAttention
...
TOTAL_RATIO = 0.2 # total % data for train+test
...
def subgraph_label_class_range(
    ...
    """
    New task: range over subgraph S is max(y_j) - min(y_j).
...
def train_one_run():
    ...
    # Precompute k-hop subsets once (CPU)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for t in range(0, t_end_eff):
            ...
            # for full model (metric/mixup), collect per-anchor pieces
            all_context = []
            all_query = []
            all_target = []
            ...
            for anchor_idx in anchor_indices.tolist():
                ...
                if USE_ATTN:
                    ...
                    # Diversity loss uses hard k derived from solved k (as in your code)
