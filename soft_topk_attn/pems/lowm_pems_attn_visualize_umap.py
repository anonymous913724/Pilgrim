...
# split
TRAIN_RATIO = 0.9  # 20% train, 80% test
...
def visualize(
    ...
    t_vis: int,
    device: torch.device,
    output_path: str = "pems_pilgrim_umap.png",
):
    model.eval()
    ...
    for lab, color, name in [(0.0, "#1f77b4", "label=0"), (1.0, "#d62728", "label=1")]:
        mask = labels_vis == lab
        if mask.any():
            plt.scatter(emb_2d[mask, 0], emb_2d[mask, 1], s=50, alpha=0.7, c=color, label=name)
    plt.legend(prop={'size': 16})
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.savefig(str.replace(output_path, '.png', '.pdf'), dpi=300)
    print(f"Saved UMAP plot to {output_path}")


def train_and_visualize_one_run(output_path: str = "pems_pilgrim_umap.png"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and visualize DCRNN with attention using UMAP")
    parser.add_argument("--output", type=str, default="pems_pilgrim_umap.png", help="Output path for the UMAP visualization plot")
    args = parser.parse_args()
    
    train_and_visualize_one_run(output_path=args.output)
